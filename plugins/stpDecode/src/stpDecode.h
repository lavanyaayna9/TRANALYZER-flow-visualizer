...
typedef union {
    uint64_t bid;
    struct {
#define STP_PRIO(prioExt) (uint16_t)(ntohs(prioExt) & 0xf000)
#define STP_EXT(prioExt)  (uint16_t)(ntohs(prioExt) & 0x0fff)
        uint16_t prioExt;
...
typedef struct {
    ...
    union {
        ...
        struct {
#define STP_ROOT_PRIO(stp) (uint16_t)(ntohs((stp)->rootPrioExt) & 0xf000)
#define STP_ROOT_EXT(stp)  (uint16_t)(ntohs((stp)->rootPrioExt) & 0x0fff)
            uint16_t rootPrioExt;     // Root Bridge Priority and Root Bridge System ID Extension
            uint8_t  rootHW[ETH_ALEN]; // Root Bridge System ID
        };
    };
    uint32_t rootCost; // Root Path Cost
    union {
#define STP_BRIDGE_PRIO(stp) (uint16_t)(ntohs((stp)->bridgePrioExt) & 0xf000)
#define STP_BRIDGE_EXT(stp)  (uint16_t)(ntohs((stp)->bridgePrioExt) & 0x0fff)
        uint64_t bridge;   // Bridge Identifier
        struct {
            uint16_t bridgePrioExt;     // Bridge Priority and Bridge System ID Extension
