...
#define IGMP_TC_MD     0 // type code representation:
                         //   0: bit field
                         //   1: explicit array of type code [NOT IMPLEMENTED YET]
                         //   2: type code statistics [NOT IMPLEMENTED YET]
//#define IGMP_NUM      10 // if IGMP_TC_MD == 1: number of type info / flow
#define IGMP_STATFILE  1 // Print global IGMP statistics in a separate file

/* +++++++++++++++++++++ ENV / RUNTIME - conf Variables +++++++++++++++++++++ */

#define IGMP_NOCODE "-"              // Symbol to use to represent the absence of a code
#define IGMP_SUFFIX "_igmpStats.txt" // Suffix for output file
...
typedef struct igmpDecodeFlow_s {
    int8_t version;
    ip4Addr_t mcast_addr;
#if IGMP_TC_MD == 0
    uint32_t type_bfield;
    //uint16_t igmp_code_bfield;
//#elif IGMP_TC_MD == 1
//    uint8_t type[IGMP_NUM];
//    //uint8_t code[IGMP_NUM];
//#elif IGMP_TC_MD == 2
//    uint8_t type[IGMP_TYPEFIELD];
//    //uint8_t code[IGMP_CODEFIELD];
#endif // IGMP_TC_MD == 2
    uint8_t stat;
    //uint8_t rec_type;
    uint16_t nrec;  // number of group records (IGMP_V3_MEMBERSHIP_REPORT)
