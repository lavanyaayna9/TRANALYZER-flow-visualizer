...
#define RADIUS_CNTS      1 // Output counts (required for FORCE_MODE)
...
#define RAD_CNTMX       20 // Max number of codes/AVP types
#define RADIUS_STRMAX  128 // Max length for strings

/* +++++++++++++++++++++ ENV / RUNTIME - conf Variables +++++++++++++++++++++ */

/*       No env / runtime configuration flags available for radiusDecode      */
...
// 28 (Reserved)
#define RADIUS_C_NEX            29 // Next-Passcode   [RFC3575]
...
const char * const avpNM[] = {
    ...
    "Login-LAT-Node",
    "Login-LAT-Group",
    "Framed-AppleTlk-Link",
    "Framed-AppleTlk-Net",
    "Framed-AppleTlk-Zone",
    "Acct-Status-Type",
    "Acct-Delay-Time",
    "Acct-Input-Oct",
    "Acct-Output-Oct",
    ...
    "Tunnel-Type",
    "Tunnel-Medium-Type",
    "Tunnel-Client-Endpnt",
    "Tunnel-Server-Endpnt",
    "Acct-Tunnel-Conn",
    "Tunnel-Password",
    "ARAP-Password",
    "ARAP-Features",
    ...
    "EAP-Msg",
    "Msg-Auth",
    "Tunnel-Priv-Group-ID",
    "Tunnel-Assign-ID",
    "Tunnel-Pref",
    "ARAP-Chall-Res",
    "Acct-Int-Interval",
    "Acct-Tunnel-Pkts-Lst",
    "NAS-Port-ID",
    "Framed-Pool",
    "CUI",
    ...
    "Location-Inf",
    "Location-Data",
    "Basic-Locn-Policy-Rules",
    "Ext-Loc-Policy-Rules",
    "Loc-Capable",
    "Req-Loc-Info",
    "Framed-Mngmnt-Proto",
    "Mngmnt-Trans-Protect",
    "Mngmnt-Policy-Id",
    "Mngmnt-Priv-Lvl",
    "PKM-SS-Cert",
    "PKM-CA-Cert",
    ...
    "Mobile-Node-Ident",
    "Service-Selection",
    "PMIP6-Home-LMA-IPv6-Addr",
    "PMIP6-Visited-LMA-IPv6-Addr",
    "PMIP6-Home-LMA-IPv4-Addr",
    "PMIP6-Visited-LMA-IPv4-Addr",
    "PMIP6-Home-HN-Pref",
    "PMIP6-Visited-HN-Pref",
    "PMIP6-Home-Interface-ID",
    "PMIP6-Visited-Interface-ID",
    "PMIP6-Home-IPv4-HoA",
    "PMIP6-Visited-IPv4-HoA",
    "PMIP6-Home-DHCP4-Server-Addr",
    "PMIP6-Visited-DHCP4-Server-Addr",
    "PMIP6-Home-DHCP6-Server-Addr",
    "PMIP6-Visited-DHCP6-Server-Addr",
    "PMIP6-Home-IPv4-Gateway",
    "PMIP6-Visited-IPv4-Gateway",
    "EAP-Lower-Layer",
    "GSS-Acceptor-Service-Name",
    "GSS-Acceptor-Host-Name",
    "GSS-Acceptor-Service-Spec",
    "GSS-Acceptor-Realm-Name",
    "Framed-IPv6-Addr",
    "DNS-Server-IPv6-Addr",
    "Route-IPv6-Inf",
    "Delegated-IPv6-Pref-Pool",
    "Stateful-IPv6-Addr-Pool",
    "IPv6-6rd-Config",
...
typedef struct {
    ...
    char nasid[RADIUS_STRMAX+1];
    char nasportid[RADIUS_STRMAX+1];
    uint32_t nasip;
    uint32_t nasport;
    uint32_t nasporttype;
#endif // RADIUS_NAS == 1
    ...
    uint32_t tunnelPref;
    char tunnelCli[RADIUS_STRMAX+1];
    char tunnelSrv[RADIUS_STRMAX+1];
    char tunnelCliAId[RADIUS_STRMAX+1];
    char tunnelSrvAId[RADIUS_STRMAX+1];
#endif // RADIUS_TUNNEL == 1

#if RADIUS_ACCT == 1
    // Accounting
    char acctSessId[RADIUS_STRMAX+1];
    uint32_t acctStatType;
    uint32_t acctTerm;
    uint32_t in_oct;
    uint32_t in_pkt;
    uint32_t in_gw;
    ...
    uint32_t sessTime; // how many seconds the user has received service for
    //uint64_t acct_evt_ts;

    char connInfo[RADIUS_STRMAX+1]; // user's connection
    char filter[RADIUS_STRMAX+1];
    char callingid[RADIUS_STRMAX+1];
    char calledid[RADIUS_STRMAX+1];

    uint8_t stat;
    uint8_t code;
#if RADIUS_NMS > 0
    uint8_t rcode[RAD_CNTMX];
#endif // RADIUS_NMS > 0
#if RADIUS_CNTS == 1
    ...
    char user[RADIUS_STRMAX+1];
    char replymsg[RADIUS_STRMAX+1];
    char upw[RADIUS_STRMAX+1];
#if RADIUS_AVPTYPE == 1
