...
//LOCAL int MMDB_find_address_in_search_tree(const MMDB_s *const mmdb,
//                                      uint8_t *address,
    ...
    int *const mmdb_error)
{
    ...
    *mmdb_error =
        MMDB_find_address_in_search_tree(mmdb, address, sockaddr->sa_family,
                                    &result);

    return result;
}

int MMDB_find_address_in_search_tree(const MMDB_s *const mmdb,
                                      uint8_t *address,
                                      sa_family_t address_family,
                                      MMDB_lookup_result_s *result)
{
    ...
    int *status)
{
    ...
    case MMDB_DATA_TYPE_UINT128:
        ...
        fprintf(stream, "0x%si;", hex_string);
        free(hex_string);
#else
        ...
        fprintf(stream, "0x%016" PRIX64 "%016" PRIX64 ";", high,
                low);
#endif
        entry_data_list = entry_data_list->next;
        break;
