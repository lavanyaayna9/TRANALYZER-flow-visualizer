...
#define DHCPOPTTRUNC    0x0100 // option list truncated... increase DHCPOPTMAX or DHCPMSGMAX
#define DHCPNMTRUNC     0x0200 // domain or host name list truncated... increase DHCPNMMAX
#define DHCPMSGTPUNK    0x0800 // Warning: unknown message type
...
#else // DHCPMOTOUT > 0
    uint16_t msgTNum;
    uint8_t msgT[DHCPMSGMAX];
#endif // DHCPMOTOUT

#if IPV6_ACTIVATE == 0 || IPV6_ACTIVATE == 2
#if DHCPMOTOUT > 0
    uint16_t optNum;
    uint8_t opt[DHCPOPTMAX];
#endif // DHCPMOTOUT > 0
    char serverName[64];
    char bootFile[128];
    char msg[256];
