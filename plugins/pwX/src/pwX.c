...
T2_PLUGIN_INIT("pwX", "0.9.3", 0, 9);


#if (PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_HTTP) != 0
/**
...
#endif // (PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_HTTP) != 0
 ...
 */
#if (PWX_PAP | PWX_HTTP_BASIC | PWX_HTTP_PROXY | PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_TELNET | PWX_LDAP) != 0
...
#endif // (PWX_PAP | PWX_HTTP_BASIC | PWX_HTTP_PROXY | PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_TELNET | PWX_LDAP) != 0


/*
 * Set auth status in opposite flow and mark this flow as extracted.
 */
#if PWX_STATUS != 0 && (PWX_PAP | PWX_HTTP | PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_LDAP) != 0
...
#endif // PWX_STATUS != 0 && (PWX_PAP | PWX_HTTP | PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_LDAP) != 0


#if (PWX_SMTP | PWX_IMAP | PWX_HTTP) != 0
/**
 * Make a copy of username and password buffers and store them in the password flow structure.
...
#endif // (PWX_SMTP | PWX_IMAP | PWX_HTTP) != 0
...
static int extract_telnet(char **buffer, const char *src, const char * const end) {
    ...
    if (!*buffer) {
        *buffer = t2_malloc(PE_BUFFER_SIZE);
...
static bool pap_extract(pwX_flow_t * const pwx_flow, const uint8_t *pap_hdr, size_t pap_len) {
    ...
    const size_t user_len = (size_t)pap_hdr[4];
    ...
    // copy username
    if (!(pwx_flow->username = t2_malloc(user_len + 1))) {
        return false;
    }
    ...
    const size_t pass_len = (size_t)pap_hdr[user_len + 5];
    if (pass_len + user_len + 6 > pap_len) {
        return false;
    }

    // copy password
    if (!(pwx_flow->password = t2_malloc(pass_len + 1))) {
        return false;
    }
 ...
 */
static PasswordProtocol proto_from_port(
    uint16_t dstPort
#if (PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_HTTP | PWX_TELNET | PWX_LDAP) == 0
        UNUSED
#endif
    , uint16_t srcPort
#if PWX_TELNET == 0
...
void t2Init() {
    ...
    T2_PLUGIN_STRUCT_NEW(pwX_flows);
}
...
binary_value_t* t2PrintHeader() {
    ...
    BV_APPEND_U8(bv, "pwxType", "Authentication type of the extracted username/password");
#if PWX_USERNAME != 0
    BV_APPEND_STR(bv, "pwxUser", "Extracted username");
#endif // PWX_USERNAME != 0
#if PWX_PASSWORD != 0
    BV_APPEND_STR(bv, "pwxPass", "Extracted password");
#endif // PWX_PASSWORD != 0
#if PWX_STATUS != 0
    BV_APPEND_U8(bv, "pwxStatus", "Authentication status");
#endif // PWX_STATUS != 0
...
void t2OnLayer2(packet_t* packet, unsigned long flowIndex) {
    ...
    if (passExtractP->proto == UNDEFINED) {
        ...
        if ((packet->status & L2_PPPoE_S) && (packet->pppoeHdrP != NULL) &&
                (packet->pppoeHdrP->pppProt == PPP_PAPn)) {
            // check that packet is not snapped before PAP header
            const uint8_t * const pap_hdr = (uint8_t *)packet->pppoeHdrP + sizeof(pppoEH_t);
            if (pap_hdr - (uint8_t *)packet->l2HdrP < packet->snapL2Len) {
                const size_t pap_len = packet->snapL2Len - (pap_hdr - (uint8_t *)packet->l2HdrP);
...
void t2OnLayer4(packet_t* packet, unsigned long flowIndex) {
    ...
    if (passExtractP->proto == UNDEFINED) {
        ...
        if ((packet->status & L2_PPP) && (packet->status & L2_GRE) &&
                (packet->pppHdrP != NULL) && (packet->pppHdrP->pppHdru.prot == PPP_PAPn)) {
            // check that packet is not snapped before PAP header
            const uint8_t * const pap_hdr = (uint8_t *)packet->pppHdrP + sizeof(pppHdr_t);
            if (pap_hdr - (uint8_t *)packet->l2HdrP < packet->snapL2Len) {
                const size_t pap_len = packet->snapL2Len - (pap_hdr - (uint8_t *)packet->l2HdrP);
                if (pap_extract(passExtractP, pap_hdr, pap_len)) {
...
#if (PWX_FTP | PWX_POP3 | PWX_IRC | PWX_SMTP | PWX_IMAP | PWX_HTTP | PWX_TELNET | PWX_LDAP) != 0
    // L7 payload
    const char * payload = (const char * const) packet->l7HdrP;
    const char * const payload_end = payload + packet->snapL7Len;
#endif

    // temporary buffers
#if (PWX_SMTP | PWX_IMAP | PWX_HTTP | PWX_IRC | PWX_FTP | PWX_POP3) != 0
    char buffer[PE_BUFFER_SIZE];
#endif // (PWX_SMTP | PWX_IMAP | PWX_HTTP | PWX_IRC | PWX_FTP) != 0
#if (PWX_SMTP | PWX_IMAP | PWX_HTTP) != 0
    ...
    if (passExtractP->proto == IRC && packet->snapL7Len > 5) {
        ...
        return;
    #if PWX_STATUS != 0
    } else if (passExtractP->proto == CHECK_IRC_FREENODE && packet->snapL7Len > 40) {
        read_line(payload, buffer, payload_end, PE_BUFFER_SIZE);
    ...
    if (passExtractP->proto == SMTP) {
        // start of AUTH LOGIN
        if (packet->snapL7Len >= 10 && strncmp(payload, "AUTH LOGIN", 10) == 0) {
            passExtractP->smtp_login_state = 1; // next packet is username
            passExtractP->next_seq = seq + packet->l7Len;
        } else if (packet->snapL7Len >= 10 && strncmp(payload, "AUTH PLAIN", 10) == 0) {
        ...
        } else if (packet->snapL7Len >= 3 && passExtractP->smtp_login_state == 1 &&
        ...
        } else if (packet->snapL7Len >= 3 && passExtractP->smtp_login_state == 2 &&
                seq == passExtractP->next_seq) {
            read_line(payload, buffer, payload_end, PE_BUFFER_SIZE);
            ...
            passExtractP->proto = ALREADY_EXTRACTED;
            set_check_status(flowIndex, CHECK_SMTP);
        } else if (packet->snapL7Len >= 3 && passExtractP->smtp_plain_state == 1 &&
                seq == passExtractP->next_seq) {
            read_line(payload, buffer, payload_end, PE_BUFFER_SIZE);
            decode_plain_auth(buffer, user_buffer, pass_buffer);
            // store username and password in flow and set auth as SMTP
        ...
        return;
    #if PWX_STATUS != 0
    } else if (passExtractP->proto == CHECK_SMTP && packet->snapL7Len >= 3) {
    ...
    if (passExtractP->proto == IMAP && packet->snapL7Len >= 10) {
        ...
        // check that the LOGIN string is present and correctly positioned
        ...
        return;
    #if PWX_STATUS != 0
    } else if (passExtractP->proto == CHECK_IMAP && packet->snapL7Len > 5) {
        const char * const space = strnstr(payload, " ", packet->snapL7Len);
    ...
    if (passExtractP->proto == HTTP) {
        ...
        return;
    #if PWX_STATUS != 0
    } else if (passExtractP->proto == CHECK_HTTP && packet->snapL7Len >= 12) {
    ...
    if (passExtractP->proto == TELNET) {
        if (passExtractP->telnet_state == 1 && passExtractP->next_seq == seq) {
            if (extract_telnet(&passExtractP->username, payload, payload_end)) {
                ...
                passExtractP->next_seq += packet->l7Len;
            }
...
#if PWX_LDAP != 0
    if (passExtractP->proto == LDAP && packet->snapL7Len > 5) {
    ...
    #if PWX_STATUS != 0
    } else if (passExtractP->proto == CHECK_LDAP && packet->snapL7Len >= 5) {
...
void t2OnFlowTerminate(unsigned long flowIndex, outputBuffer_t *buf) {
    ...
    OUTBUF_APPEND_U8(buf, passExtractP->auth_type);
    ...
    OUTBUF_APPEND_U8(buf, passExtractP->status);
#endif // PWX_STATUS != 0
}
...
void t2PluginReport(FILE *stream) {
    if (pwx_count) {
        ...
        T2_FPLOG(stream, plugin_name,
                "Number of passwords with successful/failed/unknown login: %" PRIu32 "/%" PRIu32 "/%" PRIu32,
