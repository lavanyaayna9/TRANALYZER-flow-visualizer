...
#include <pcre.h>     // for pcre, pcre_extra
...
// local defines
#define HDRSELMX  5 // max dimension of the header features to select from packet:
                    // flowdir, prot, srcIP, dstIP + select bits < 27

#define HDRSELMSK (1U << (HDRSELMX-2))
#define PCREMDMSK 0x08000000

#define BUFREGMAX  500               // max length of regex rule
#define BUFPREIDMX ((PREIDMX+5)* 10) // max length of regex rule

/* regex pcre library Options. Some are compile-time only, some are run-time only, and some are
...
typedef struct {
    ...
    pcre_extra **studyRex;
#endif
    uint32_t *hdrSel[HDRSELMX];
    uint16_t *preID[PREIDMX];
    uint16_t *id;
    uint16_t *flags;
    uint16_t *offset;
    uint8_t  *alarmcl;
