...
#define PSI_XCLD               0 // 1: include (PSI_XMIN, UINT16_MAX]
...
#define HISTO_EARLY_CLEANUP    0 // 1: after t2OnFlowTerminate tree information is destroyed
...
#define PSIAT_NDPLF    17 // multiplication factor red-black tree nodepool:
                          // sizeof(nodepool) = PSIAT_NDPLF * mainHashMap->hashChainTableSize
...
#if TSTAMP_PREC == 1
#define IATNORM      1000000 // select ns as basic unit
#else // TSTAMP_PREC == 0
...
typedef struct {
    uint32_t        numPackets;                      // number of packets inside the tree
    rbNode_t       *packetTree;                      // root of the stored tree
    struct timeval  lastPacketTime;
    uint32_t        numPacketsInTimeBin[IATBINUMAX]; // stores the packets with a specific IAT, so we don't
                                                     // need to traverse the tree several times
} pktSIAT_t;
