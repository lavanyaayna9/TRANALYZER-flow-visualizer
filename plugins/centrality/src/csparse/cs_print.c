...
csi cs_print (const cs *A, csi brief, FILE *matrixFile)
{
    ...
    {
        //fprintf (stdout, "Matrix triplet: %g-by-%g, nzmax: %g nnz: %g\n", (double) m,
        //    (double) n, (double) nzmax, (double) nz) ;
        for (p = 0 ; p < nz ; p++)
        {
           fprintf (matrixFile, "%g %g %g\n", (double) (Ai [p]+0), (double) (Ap [p]+0),
                Ax ? Ax [p] : -1) ;
            //if (brief && p > 20) { printf ("  ...\n") ; return (1) ; }
        }
    }
