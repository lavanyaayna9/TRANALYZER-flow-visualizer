...
#define PKTSXER_V2_MAGIC   0x3252455853544b50 // PKTSXER2 : packet mode (t2 -s)
...
typedef struct {
    char* pcap_path;
    union {
        uint64_t flow_count;    // flow mode
        uint64_t first_pkt;     // packet mode
    };
    union {
        // v1: points to the first flow_index in the PCAP header
        // v2: points to the first flow_header
        uint64_t flow_pointer;  // flow mode
        uint64_t last_pkt;      // packet mode
    };
...
// _flows.xer / _packets.xer version
typedef enum {
    XER_INVALID,
    FINDEXER_V1,
    FINDEXER_V2,
    // PKTSXER_V1 does not exist
    PKTSXER_V2,
} XerVersion;
...
typedef struct {
    uint64_t* pkts;                 // list of packets to extract
    uint64_t count;                 // size of packets list
    uint64_t allocated;             // allocated size of packets list
    uint64_t pos;                   // position of last read packet number
} pkt_list_t;
...
static bool first_pkt_mode = false; // print first packet PCAP and timestamp and exit (-n option)
...
static XerVersion xer_version;      // v1 or v2 (see doc/findexer.pdf for format details)

static bool packet_mode = false;    // extract specific packets instead of whole flows
static pkt_list_t packets;          // packets to extract
...
void add_packet_to_extract(uint64_t pktno) {
    // reallocate memory if packet list is full
    if (packets.count >= packets.allocated) {
        packets.allocated <<= 1;
        uint64_t* tmp = realloc(packets.pkts, packets.allocated * sizeof(*packets.pkts));
        if (!tmp) {
            free(packets.pkts);
            FATAL("Failed to re-allocate memory for packet list.");
        }
        packets.pkts = tmp;
    }
    // put flow in list
    packets.pkts[packets.count++] = pktno;
}
...
void read_flow_file(char* filename) {
    ...
    while ((read = getline(&line, &len, file)) != -1) {
        ...
        if ((tmp = splitstr(tmp, '\t'))) {
            ...
            if (!both_dir) {
                int8_t flowInvert = -1;
                if ((tmp = splitstr(tmp, '\t')) && tmp[0] == '0' && tmp[1] == 'x') {
                    // acccess the next token (flowStat)
                    const uint64_t flowStat = strtoull(tmp, NULL, 16);
                    flowInvert = (flowStat & 1);  // extract L3FLOWINVERT bit
                }
                if (flowInvert == 0 || strcmp(line, "A") == 0) {
                    dir = DIR_A;
                } else if (flowInvert == 1 || strcmp(line, "B") == 0) {
                    dir = DIR_B;
                } else {
...
void read_packet_file(char* filename) {
    ...
    if (strcmp("-", filename) == 0) {
        ...
        FATAL("Failed to open packet file %s", filename);
    }
    ...
    while ((read = getline(&line, &len, file)) != -1) {
        ...
        // must not modify line otherwise next getline call will crash
        char* tmp = line;
        // split at tab in case there are multiple columns
        splitstr(tmp, '\t');

        errno = 0;
        char* end;
        uint64_t pktno = strtoull(tmp, &end, 10);
        if (errno != 0 || pktno == 0 || *end != '\0') {
            FATAL("Invalid value: \"%s\": PKTNO must be a strictly positive 64-bit number", tmp);
        }
        add_packet_to_extract(pktno);
    }
    fclose(file);
    free(line);
}
...
void parse_args(int argc, char** argv) {
    ...
    while ((c = getopt(argc, argv, ":hfPnbw:r:R:p:s:i:")) != -1) {
        switch (c) {
            case 'h':
                print_usage();
                exit(EXIT_SUCCESS);
            case 'r': {
                ...
                memcpy(findexer_file_path, optarg, len);
    ...
    if (!first_pkt_mode && !output_file_path) {
        FATAL("Missing mandatory -w or -n option.\n"
              "      Run 'fextractor -h' for more information.");
    }
    // parse flow/packet file
    if (flow_file) {
        if (packet_mode) {
            read_packet_file(flow_file);
        } else {
            read_flow_file(flow_file);
        }
    }
    // parse flow indexes / packet numbers passed as argument
    for (c = optind; c < argc; ++c) {
        ...
        if (packet_mode) {
            int64_t pktno = strtoull(argv[c], &endc, 10);
            if (errno != 0 || pktno == 0 || endc != findex_end || pktno < 0) {
                FATAL("Invalid value: \"%s\": PKTNO must be a strictly positive 64-bit number", argv[c]);
            }
            add_packet_to_extract(pktno);
            continue;
        }
    ...
    if (!first_pkt_mode) {
        if (packet_mode && packets.count == 0) {
            FATAL("Requires at least one PKTNO.\n"
                  "      Run 'fextractor -h' for more information.");
        } else if (!packet_mode && flow_count == 0) {
...
XerVersion read_version(FILE* f) {
    // go to start of file
    ...
    if (packet_mode) {
        switch(magic) {
            case PKTSXER_V2_MAGIC:
                return PKTSXER_V2;
            default:
                // TODO: handle opposite endianness packets.xer files
                FATAL("Input file is not a valid packet findexer file.");
        }
    } else {
        switch(magic) {
            ...
            default:
                ...
                FATAL("Input file is not a valid flow findexer file.");
        }
    }
    return XER_INVALID;
}
...
uint64_t next_pcap_header_pos(FILE* f, uint32_t pcap_index, uint64_t last_pcap_header_pos) {
    ...
    switch(xer_version) {
        ...
        case PKTSXER_V2:
            ...
            break;
        default:
            FATAL("Unsupported findexer version: %d.", xer_version);
    }
...
findexer_pcap_header_t* read_pcap_header(FILE* f, uint64_t offset) {
    ...
    if (xer_version == FINDEXER_V2 || xer_version == PKTSXER_V2) {
...
int flow_order(const void* a, const void* b) {
    ...
    if (fa->findex > fb->findex) {
        ...
        // WARNING: DO NOT use xer_version as the _flows.xer file has not been read yet.
        if (fa->dir > fb->dir) {
            return 1;
...
/*
 * Comparator to sort packet numbers with qsort.
 */
int pkt_order(const void* a, const void* b) {
...
int flow_in_list(const void* a, const void* b) {
    ...
    if (fa->findex > fb->findex) {
        ...
        if (xer_version == FINDEXER_V1 || fb->dir == DIR_BOTH || fa->dir == fb->dir) {
            return 0;
        } else if (fa->dir > fb->dir) {
            return 1;
        } else { // fa->dir < fb->dir
...
bool extract_pkt(FILE *pcap, uint64_t offset, bool opposite_endianness, findexer_pcap_header_t* pcapHeader) {
...
bool process_findexer_file(char* path, bool warning) {
    ...
    xer_version = read_version(input);
    ...
    for (uint32_t i = 0; i < pcap_count; ++i) {
        if (!first_pkt_mode && \
                ((!packet_mode && flow_count == 0) \
                  || (packet_mode && packets.pos >= packets.count))) {
            break;
        }
        ...
        if (pcap_header_buffer[0] == bswap_32(PCAP_MAGIC) ||
            pcap_header_buffer[0] == bswap_32(PCAP_MAGIC_NS))
        {
            opposite_endianness = true;
        } else if (pcap_header_buffer[0] != PCAP_MAGIC &&
                   pcap_header_buffer[0] != PCAP_MAGIC_NS)
        {
        ...
        if (first_pkt_mode) {
            ...
            if (fraction >= 1.0) {
                // unreliable method to detect if PCAP was recorded with micro- or nano-second precision
        ...
        if (packet_mode) {
            // check if any packet to extract in current PCAP
            if (packets.pkts[packets.pos] > pcapHeader->last_pkt \
                    || pcapHeader->first_pkt > packets.pkts[packets.count - 1]) {
                continue;
            }
        } else {
        ...
        if (packet_mode) {
            while (packets.pos < packets.count && packets.pkts[packets.pos] < pcapHeader->first_pkt) {
                packets.pos++; // skip missing packets
            }
            // extract packets
            size_t last_pos = 0; // last extracted packet position in PCAP header packet buffer
            while (packets.pos < packets.count && packets.pkts[packets.pos] < pcapHeader->last_pkt) {
                const size_t toskip = (packets.pkts[packets.pos] - pcapHeader->first_pkt) * sizeof(offset) - last_pos;
                // read next packet offset
                if (fseeko(input, toskip, SEEK_CUR) != 0) {
                    FATAL("Failed to seek to next packet offset: %zu\n", last_pos + toskip);
                }
                if (fread(&offset, sizeof(offset), 1, input) != 1) {
                    FATAL("Failed to read packet offset.");
                }
                packets.pos++;
                last_pos += toskip + sizeof(offset);
                // TODO: extract packets in packet_mode
                if (!extract_pkt(pcap, offset, opposite_endianness, pcapHeader)) {
...
int main(int argc, char** argv) {
    ...
    // create the list of packets to extract (-P mode)
    if (!(packets.pkts = calloc(INITIAL_FLOW_COUNT, sizeof(*packets.pkts)))) {
        FATAL("Failed to allocate memory for packet list.");
    }
    packets.allocated = INITIAL_FLOW_COUNT;
    packets.count = 0;
    packets.pos = 0;
    // parse the command line arguments
    parse_args(argc, argv);

    // sort the flows / packet indices
    if (packet_mode) {
        qsort(packets.pkts, packets.count, sizeof(*packets.pkts), pkt_order);
    } else {
    ...
    if (multi_start_index == -1 && multi_end_index == -1 && access(findexer_file_path, F_OK) != -1) {
        ...
        while (multi_end_index == -1 || (int64_t)multi_current_index <= multi_end_index) {
            ...
            if (!first_pkt_mode && flow_count == 0) {
