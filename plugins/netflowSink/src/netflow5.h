...
typedef struct {
    ...
    uint32_t SysUptime;         // Device system uptime. Applies only to routers and so. Therefore will be 0
    uint32_t unix_secs;         // Actual time in seconds since epoch
    uint32_t unix_nsecs;        // Residual nanoseconds since epoch
    uint32_t flow_sequence;     // Sequence number of flows actual seen. Maybe applicable. Else it will be zero. Maybe use flow index L4
