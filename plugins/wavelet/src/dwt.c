...
/*
 * These functions compute the decomposition coefficients for all the levels
 */
static inline uint16_t convolution1D_full(WPREC *X, int dimX, WPREC *H, int dimH, WPREC *Y, int dimY);
//static inline uint16_t convolution1D_valid(WPREC *X, int dimX, WPREC *H, int dimH, WPREC *Y, int dimY);
...
/*      for (i = 0; i < wave_level; i++) {
            ...
            // Details
            convolution1D_valid(ext_sig, EXT_signal_dim, hi_d, wave_len, conv_sign, conv_sig_dim);

            // Approximation
            conv_sig_dim = ext_signal_dim - wavelet_L + 1;
            convolution1D_valid(ext_sig, ext_signal_dim, lo_d, wave_len, conv_sign, conv_sig_dim);
        }
*/
    } else {
        ...
        for (i = 0; i < wave_level; i++) {
            // Details
            waveP->wtlvl_len[i] = convolution1D_full(sig, sig_len, hi_d, wt_len, wt_detail, sig_len);
            // Approximation
            sig_len = convolution1D_full(sig, sig_len, lo_d, wt_len, wt_approx, sig_len);
            wt_detail += sig_len;
            sig = wt_approx;
            wt_approx += sig_len;
        }
...
/*
 * FULL COMPLETE CONVOLUTION of TWO ARRAYS
 * 1D DIMENSION
 * @X: input signal
 * @H: filter
 * @Y: output signal
...
static inline uint16_t convolution1D_full(WPREC *X, int dimX, WPREC *H, int dimH, WPREC *Y, int dimY) {
    ...
    for (int i = 0; i < dimY; i++) {
        ...
        for (uint_fast16_t j = j1; j <= j2; j++) s += X[i-j] * H[j];
 ...
 * 1D DIMENSION
 * compute only those parts of the convolution that are computed without the padded edges.
...
/*static inline uint16_t convolution1D_valid(WPREC *X, int dimX, WPREC *H, int dimH, WPREC *Y, int dimY) {
    ...
    uint16_t j1, ja1, j2, ja2, k = 0;

    for (int i = 0; i < dimY; i++) {
        WPREC s = 0.0;

        const uint16_t j = i + dimH - 1;
        ...
        for (uint_fast16_t ja = ja1; ja <= ja2; ja++) s += X[ja] * H[jp1-ja-1];
