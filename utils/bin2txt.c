...
#include <inttypes.h>             // for PRIu8, PRIu32, PRIu64, PRId16, PRId32
#include <math.h>                 // for ceilf
#include <netinet/in.h>           // for INET6_ADDRSTRLEN
#include <stdbool.h>              // for false, bool, true
#include <stdint.h>               // for uint_fast32_t, UINT32_MAX, int8_t, uint8_t, ...
#include <stdio.h>                // for FILE, fread, fclose, fgetc, fopen
#include <string.h>               // for NULL, size_t, strchr, strlen
#include <sys/socket.h>           // for AF_INET6
#include <time.h>                 // for strftime, gmtime, localtime, time_t

#include "bin2txt.h"              // for b2t_func_t, B2T_PRIX8, JSON_PRETTY_STR
#include "binaryValue.h"          // for binary_subvalue_t, binary_value_t
#include "flow.h"                 // for FLOW_DIR_C_A, FLOW_DIR_C_B
#include "gz2txt.h"               // for USE_ZLIB
#include "outputBuffer.h"         // for outputBuffer_t
#include "t2log.h"                // for T2_DBG, T2_ERR, T2_PERR
#include "t2utils.h"              // for t2_malloc_fatal, UNLIKELY, LIKELY, UNUSED
...
static bool parse_binary_value(void *input, binary_type_t type, void *outfile, bool json, b2t_func_t funcs);
...
// functions
static inline bool get_val_from_input_buffer(void *input, void *dest, size_t size, size_t n, b2t_func_t funcs UNUSED) {
    ...
    if (UNLIKELY(buffer->size < buffer->pos + sn)) {
        ...
        T2_PERR("bin2txt", "Buffer overflow: %zu increase MAIN_OUTBUF_SIZE in tranalyzer.h", required);
        return false;
    }
 ...
 */
void print_values_description(binary_value_t *bv, void *outfile, b2t_func_t funcs) {

    funcs.fputs("# Col No.\t", outfile);
#if TFS_NC_TYPE > 0
    funcs.fputs("Type\t", outfile);
#endif
    funcs.fputs("Name\tDescription", outfile);
    ...
    while (bv) {
        ...
        // Column type
        for (i = 0; i < bv->num_values; i++) {
            if (i != 0) funcs.fputc('_', outfile); // Separator
            if (bv->subval[i].type == bt_compound) {
                parse_subheader_type(&(bv->subval[i]), outfile, funcs);
            } else {
#if TFS_NC_TYPE == 2
                funcs.fputs(type2str[bv->subval[i].type], outfile);
#else // TFS_NC_TYPE == 1
                funcs.fprintf(outfile, "%" PRIu32, bv->subval[i].type);
#endif // TFS_NC_TYPE
            }
        }

        // Repetitive flag
        if (bv->is_repeating) funcs.fputs(":R", outfile);
        //else funcs.fputs(":N", outfile);

        // Column name and description
        funcs.fprintf(outfile, "\t%s\t%s", bv->name, bv->desc);
...
void parse_binary_header2text(binary_value_t * const bv, void *outfile, b2t_func_t funcs) {
    ...
    while (bvp) {
        if (bvp != bv) funcs.fputs(SEP_CHR, outfile);
        for (i = 0; i < bvp->num_values; i++) {
            if (i != 0) funcs.fputc('_', outfile); // Separator
            if (bvp->subval[i].type == bt_compound) {
                parse_subheader_type(&(bvp->subval[i]), outfile, funcs);
            } else {
                funcs.fprintf(outfile, "%" PRIu32, bvp->subval[i].type);
            }
        }

        // Repetitive flag
        if (bvp->is_repeating) funcs.fputs(":R", outfile);
...
static void parse_subheader_type(const binary_subvalue_t * const sv, void *outfile, b2t_func_t funcs) {
    ...
    // Print repetitive flag and closing parenthesis
...
static inline void json_print_value_name(const binary_value_t * const act_bv, const binary_value_t * const bv, void *outfile, b2t_func_t funcs) {
    if (LIKELY(act_bv != bv)) {
        funcs.fputs("," JSON_PRETTY_STR, outfile);
    }
    // Write object name
    funcs.fprintf(outfile, "\"%s\":" JSON_PRETTY_STR, act_bv->name);
}
...
inline bool parse_file_bin2txt(FILE *input, binary_value_t * const bv, FILE *outfile, bool compress) {
    ...
    funcs.fread = (fread_func_t)fread;
...
bool parse_binary2text(void *input, binary_value_t * const bv, void *outfile, b2t_func_t funcs) {
    ...
    binary_value_t *bvp = bv;
    while (bvp) {
        if (bvp != bv) funcs.fputs(SEP_CHR, outfile); // Column separator
        ...
        for (rep = 0; rep < num_repeat; rep++) {
            ...
            for (val = 0; val < bvp->num_values; val++) {
                if (bvp->subval[val].type != bt_compound) {
                    if (UNLIKELY(!parse_binary_value(input, bvp->subval[val].type, outfile, false, funcs))) {
                        return false;
                    }
                } else {
                    if (UNLIKELY(!parse_subval(input, &bvp->subval[val], outfile, funcs))) {
                        return false;
                    }
                }

                // Compound value separator
                if (val < bvp->num_values - 1) funcs.fputc('_', outfile);
            }
...
bool parse_binary2json(void *input, binary_value_t * const bv, void *outfile, b2t_func_t funcs) {
    ...
    while (bvp) {
        ...
        if (bvp->is_repeating) {
            ...
            json_print_value_name(bvp, bv, outfile, funcs);
            funcs.fputc('[', outfile);
        } else {
            num_repeat = 1;
            json_print_value_name(bvp, bv, outfile, funcs);
        ...
        for (rep = 0; rep < num_repeat; rep++) {
            ...
            for (val = 0; val < bvp->num_values; val++) {
                ...
                if (bvp->subval[val].type == bt_compound) {
                    if (UNLIKELY(!parse_subval_json(input, &bvp->subval[val], outfile, funcs))) {
                        return false;
                    }
                } else {
                    if (UNLIKELY(!parse_binary_value(input, bvp->subval[val].type, outfile, true, funcs))) {
                        return false;
                    }
                }
            }
...
#if JSON_KEEP_EMPTY == 0
        if (bvp->is_repeating == 1 && num_repeat > 0) {
#else // JSON_KEEP_EMPTY != 0
        if (bvp->is_repeating == 1) {
#endif // JSON_KEEP_EMPTY != 0
...
static bool parse_binary_value(void *input, binary_type_t type, void *outfile, bool json, b2t_func_t funcs) {
    ...
    switch (type) {
        ...
        case bt_char: {
            ...
            funcs.fprintf(outfile, "%s%c%s", quote, val, quote);
            break;
        }
        ...
        case bt_string: {
            ...
            while (val != '\0') {
                ...
                switch (val) {
                    case '\b': // backspace
                        funcs.fputs("\\b", outfile);
                        break;
                    case '\f': // form feed
                        funcs.fputs("\\f", outfile);
                        break;
                    case '\n': // line feed
                        funcs.fputs("\\n", outfile);
                        break;
        ...
        case bt_mac_addr: {
            ...
            funcs.fprintf(outfile, "%" PRIu64, mac64);
#else // MAC_FORMAT == 0
            funcs.fprintf(outfile,
...
b2t_ip6:;
        ...
        case bt_ipx_addr: {
            ...
            if (version == 4) goto b2t_ip4;
                ...
                // Not IPv4, nor IPv6... print a dash instead
                funcs.fprintf(outfile, "%s%s%s", quote, B2T_NON_IP_STR, quote);
            } else {
        ...
        case bt_flow_direction: {
            ...
            funcs.fprintf(outfile, "%s%c%s", quote, (val == 0) ? FLOW_DIR_C_A : FLOW_DIR_C_B, quote);
            break;
        }
...
// Set 'magic' to the offset (number of bytes) where BV_MAGIC_VALUE is to be found.
// Return true if the magic header was found, false otherwise.
inline bool t2_get_bin_magic_offset(void *infile, b2t_func_t funcs, uint32_t *magic) {
    assert(funcs.fread && funcs.rewind && funcs.fseek);

    funcs.rewind(infile);

    uint32_t val = 0;

    // First find the offset (number of bytes) of BV_MAGIC_VALUE
    uint32_t offset = 0;
    while (UNLIKELY(funcs.fread(&val, sizeof(val), 1, infile) == 1)) {
        if (val == BV_MAGIC_VALUE_1) {
            // We found the first part of the magic value...
            // Make sure the second part is there as well...
            if (UNLIKELY(funcs.fread(&val, sizeof(val), 1, infile) != 1)) {
                T2_ERR("Invalid header: found BV_MAGIC_VALUE_1, but failed to read BV_MAGIC_VALUE_2");
                funcs.rewind(infile);
                *magic = 0;
                return false;
            }

            // We found second part of the magic value!
            if (val == BV_MAGIC_VALUE_2) break;

            // The second part of the magic value was not there...
            // It could be the first part again, so unread it and test it again
            funcs.fseek(infile, -4, SEEK_CUR);
        }

        offset += 4;

        // Make sure offset does not overflow
        if (UNLIKELY(offset == UINT32_MAX)) {
            T2_ERR("Invalid header: BV_MAGIC_VALUE was not found in the first UINT32_MAX bytes");
            funcs.rewind(infile);
           *magic = 0;
           return false;
        }
...
inline uint32_t t2_get_bin_header_len_from_magic(void *infile, b2t_func_t funcs, uint32_t magic) {
    assert(funcs.fread && funcs.fseek && funcs.rewind);

    uint32_t val1, val2;
    if (UNLIKELY(funcs.fseek(infile, magic, SEEK_SET) < 0         ||
                 funcs.fread(&val1, sizeof(val1), 1, infile) != 1 ||
                 funcs.fread(&val2, sizeof(val2), 1, infile) != 1 ||
                 val1 != BV_MAGIC_VALUE_1 || val2 != BV_MAGIC_VALUE_2))
    {
        T2_ERR("Invalid offset to magic value");
        funcs.rewind(infile);
        return 0;
    }

    uint32_t hdrlen = 8; // BV_MAGIC_VALUE_[12]

    while (LIKELY(funcs.fread(&val1, sizeof(val1), 1, infile) == 1)) {
        hdrlen += 4;

        // Make sure hdrlen does not overflow
        if (UNLIKELY(hdrlen == UINT32_MAX)) {
            T2_ERR("Invalid header: UINT32_MAX was not found in the first UINT32_MAX bytes after the magic value");
            funcs.rewind(infile);
            return 0;
        }

        if (val1 == UINT32_MAX) break; // UINT32_MAX marks the end of the header
    }
...
// Estimate the value of BUF_DATA_SHFT and the length of the preamble,
// e.g., HOST_INFO from socketSink
// Return the size of the header in 32-bits words
// Set *dataShft to the estimated value of BUF_DATA_SHFT (number of 32-bits words) and
// Set *preamble to the size of the preamble (number of bytes)
// Note that the size of the preamble includes BUF_DATA_SHFT, so to compute the
// actual size you must subtract (BUF_DATA_SHFT << 2)
inline uint32_t t2_get_bin_header_len(void *infile, b2t_func_t funcs, uint32_t *preamble, uint32_t *dataShft, uint32_t *magicOff) {
    assert(funcs.fread && funcs.rewind && funcs.fseek && funcs.ftell);

    funcs.rewind(infile);

    // First find the offset of BV_MAGIC_VALUE
    uint32_t magic = 0;
    if (!t2_get_bin_magic_offset(infile, funcs, &magic)) {
        T2_ERR("Failed to get magic header offset");
        if (magicOff) *magicOff = 0;
        *preamble = 0;
        *dataShft = 0;
        return 0;
    }

    if (magicOff) *magicOff = magic;

    // Then compute the length of the header
    const uint32_t hdrlen = t2_get_bin_header_len_from_magic(infile, funcs, magic);
    ...
    if (magic == 0) {
        // Simplest case, BUF_DATA_SHFT = 0 and no preamble
        *preamble = 0;
        *dataShft = 0;
        return hdrlen >> 2;
    }

    // Header does not start with the magic value...
    // Three cases:
    //     1) BUF_DATA_SHFT > 0, no preamble and the length is encoded at position 0
    //     2) BUF_DATA_SHFT > 0, preamble and the length is encoded at position 0
    //     3) BUF_DATA_SHFT = 0, preamble

    uint32_t val = 0;
    if (UNLIKELY(funcs.fread(&val, sizeof(val), 1, infile) != 1)) {
        // Should not happen as t2_get_bin_magic_offset() was successful
        T2_ERR("Failed to read first uint32_t word");
        funcs.rewind(infile);
        *preamble = 0;
        *dataShft = 0;
        return hdrlen >> 2;
    ...
    // Case 1: BUF_DATA_SHFT > 0 and no preamble
    if (val == hdrlen + magic) {
        funcs.rewind(infile);
        *preamble = 0;
        *dataShft = magic >> 2;
    ...
    // Read the length of the (potential) second block
    uint32_t val2 = 0;
    if (UNLIKELY(funcs.fseek(infile, val, SEEK_SET) < 0 ||
                 funcs.fread(&val2, sizeof(val2), 1, infile) != 1) ||
                 val2 != hdrlen + (magic - val))
    {
        // val was probably not the length of the first block...
        // => Case 3: BUF_DATA_SHFT = 0 and preamble > 0
        funcs.rewind(infile);
        *preamble = magic;
        *dataShft = 0;
        return hdrlen >> 2;
    ...
    // Case 2: BUF_DATA_SHFT > 0 and preamble > 0
    *preamble = val;
    *dataShft = (magic - val) >> 2;
...
// NULL is return if an error occurred
// If hdrlen is 0, the function tries to guess the length of the header
...
// If preamble is not NULL, it will be set to the inferred value of preamble (HOST_INFO from socketSink)
binary_value_t *t2_read_bin_header(void *infile, uint32_t hdrlen, b2t_func_t funcs, uint32_t *offset, uint32_t *preamble) {

    assert(funcs.fread);

    uint32_t magic = 0;
    uint32_t dataShft = offset ? *offset : 0;
    uint32_t preShft = preamble ? *preamble : 0;

    if (hdrlen == 0) {
        hdrlen = t2_get_bin_header_len(infile, funcs, &preShft, &dataShft, &magic);
        if (preamble) *preamble = preShft;
        if (offset) *offset = dataShft;
        //T2_FOK(stdout, "MAGIC: %" PRIu32 ", BUF_DATA_SHFT: %" PRIu32 ", PREAMBLE: %" PRIu32 ", HDRLEN: %" PRIu32 " (skipping %" PRIu32 ")",
        //        magic, dataShft, preShft, hdrlen, preShft + (dataShft << 2));
        //assert(preShft + (dataShft << 2) == magic);
        //funcs.fseek(infile, preShft + (dataShft << 2), SEEK_SET);
        funcs.fseek(infile, magic, SEEK_SET);
    }/* else if (UNLIKELY(hdrlen > MAIN_OUTBUF_SIZE)) {
        T2_PERR("bin2txt", "Header length is invalid: %" PRIu32 " > %d (MAIN_OUTBUF_SIZE)", hdrlen, MAIN_OUTBUF_SIZE);
        return NULL;
    }*/
    ...
    if (UNLIKELY(header[2] != BV_VERSION)) {
        T2_ERR("header version %" PRIu32 " is not supported", header[2]);
        return NULL;
    }

    uint32_t i = 3; // magic value (2) + version (1)
    uint32_t ii, iii;
    size_t len;
    float ceil_strlen;
    ...
    while (i < hdrlen - 1) {
        if (!bv) {
            bv = t2_malloc_fatal(sizeof(binary_value_t));
            act_bv = bv;
        } else {
            act_bv->next = t2_malloc_fatal(sizeof(binary_value_t));
            act_bv = act_bv->next;
        }
        act_bv->next = NULL;
        ...
        // security check
        if (UNLIKELY(i + ceil_strlen >= hdrlen || len > sizeof(act_bv->name))) {
            T2_ERR("Tranalyzer header malformed (1)");
            bv_header_destroy(bv);
            return NULL;
        }

        memcpy(act_bv->name, (char*)&header[i], len);
        ...
        // security check
        if (UNLIKELY(i + ceil_strlen >= hdrlen || len > sizeof(act_bv->desc))) {
            T2_ERR("Tranalyzer header malformed (2) (column '%s')", act_bv->name);
            bv_header_destroy(bv);
            return NULL;
        }

        memcpy(act_bv->desc, (char*)&header[i], len);

        // place i at right pos. Because a char is shorter than uint32_t it can be padded
        i += ceil_strlen;

        if (UNLIKELY(header[i] == UINT32_MAX || header[i] == 0)) {
            T2_ERR("Tranalyzer header malformed (3) (column '%s': '%s')", act_bv->name, act_bv->desc);
            bv_header_destroy(bv);
            return NULL;
        }
        ...
        act_bv->subval = t2_malloc_fatal(act_bv->num_values * sizeof(binary_subvalue_t));
        ...
        for (ii = 0; ii < act_bv->num_values; ii++) {
            ...
            if (act_bv->subval[ii].type == bt_compound) {
                ...
                // allocate space for subvalues
                act_bv->subval[ii].subval = t2_malloc_fatal(act_bv->subval[ii].num_values * sizeof(binary_subvalue_t));
...
static bool t2_read_bin_header_rec(const uint32_t * const header, uint32_t *hdrpos, binary_subvalue_t *bv) {
    ...
    // get amount of subvalues and allocate space for them
    bv->num_values = header[*hdrpos];
    bv->subval = t2_malloc_fatal(bv->num_values * sizeof(binary_subvalue_t));
