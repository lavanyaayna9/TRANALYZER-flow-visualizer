...
#if CNTYCTY == 0
#error "Set CNTYCTY to 1 in subnetHL.h: t2conf tranalyzer2 -D CNTYCTY=1"
#endif
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        printf("Usage: %s ip2locfile.bin subnetwhofile.txt\n", argv[0]);
        exit(EXIT_FAILURE);
    }
        ...
        *asnn = '\0';
        ...
        if (!subnetNr) {
            ...
            if (!subnP) {
                ...
                printf("%s/%d\t%s\t%s\t%s\t%s\t%f\t%f\t%s\t%s\t%s\t%s\t%s\n",
                        ip, msk, iprng, id, asn, p, subnP->lat, subnP->lng, subnP->loc, subnP->cnty, subnP->cty, org, addr);
            }
        }
    }
