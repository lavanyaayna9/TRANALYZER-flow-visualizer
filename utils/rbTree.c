...
#include "t2utils.h"   // for t2_[cm]alloc, UNLIKELY


static inline void rotate_left(rbNode_t *node) __attribute__((__nonnull__(1)));
static inline void rotate_right(rbNode_t *node) __attribute__((__nonnull__(1)));

static inline void insert_case1(rbNode_t *n) __attribute__((__nonnull__(1)));
static inline void insert_case2(rbNode_t *n) __attribute__((__nonnull__(1)));
static inline void insert_case3(rbNode_t *n) __attribute__((__nonnull__(1)));
static inline void insert_case4(rbNode_t *n) __attribute__((__nonnull__(1)));
static inline void insert_case5(rbNode_t *n) __attribute__((__nonnull__(1)));

static inline rbNode_t* grandparent(const rbNode_t * const node) __attribute__((__nonnull__(1)));
static inline rbNode_t* uncle(const rbNode_t * const node) __attribute__((__nonnull__(1)));

static unsigned int recursiveGetDepth(rbNode_t *tree, unsigned int currentDepth) __attribute__((__nonnull__(1)));
...
static inline void insert_case2(rbNode_t *n) {
    ...
    if (grandparent(n)) insert_case3(n);
}
...
rbTreeNodePool_t* rbTree_initTreeNodePool(uint32_t size) {
    rbTreeNodePool_t *pool = t2_malloc_fatal(sizeof(*pool));
    pool->nodePool = t2_calloc_fatal(size, sizeof(rbNode_t));
...
void rbTree_destroy(rbNode_t *tree, rbTreeNodePool_t *treeNodePool) {
    ...
    // Note: This destroying function is programmed iteratively (not recursive)
    // because of speed
...
void rbTree_print(rbNode_t *tree, unsigned long maxDepth) {
    ...
    unsigned int treeDepth = recursiveGetDepth(tree, 0);
    ...
    rbTreeNodeQueueBin_t *currentQueueBin = t2_malloc_fatal(sizeof(rbTreeNodeQueueBin_t));
    ...
    while (currentQueueBin) {
        ...
        if ((currentLevel < treeDepth) && currentQueueBin->node->left) {
            childQueueBin = t2_malloc_fatal(sizeof(rbTreeNodeQueueBin_t));
...
static unsigned int recursiveGetDepth(rbNode_t *tree, unsigned int currentDepth) {
    unsigned int maxDepth = currentDepth + 1, tempDepth;

    if (tree->left) {
        tempDepth = recursiveGetDepth(tree->left, currentDepth + 1);
        if (tempDepth > maxDepth) maxDepth = tempDepth;
    }

    if (tree->right) {
        tempDepth = recursiveGetDepth(tree->right, currentDepth + 1);
        if (tempDepth > maxDepth) maxDepth = tempDepth;
    }
