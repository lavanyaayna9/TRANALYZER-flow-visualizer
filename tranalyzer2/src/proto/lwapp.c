...
#include "lwapp.h"
#include "tranalyzer.h"      // for LWAPP
...
#include "networkHeaders.h"  // for IPV6_ACTIVATE, LLC_LEN, ethe...
...
inline uint8_t *t2_process_lwapp(uint8_t *pktptr, packet_t *packet) {
    ...
    pktptr = t2_process_ieee80211(pktptr, LWAPP_SWAP_FC, packet);
    if (!pktptr) goto lwapp_reset;

    // check for 802.1Q/ad signature (VLANs)
    _8021Q_t *shape = (_8021Q_t*)pktptr;
    if (packet->snapL2Len - skip >= sizeof(_8021Q_t)) {
