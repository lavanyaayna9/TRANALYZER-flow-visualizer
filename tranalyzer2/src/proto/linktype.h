...
#ifndef T2_LINKTYPE_H_INCLUDED
#define T2_LINKTYPE_H_INCLUDED
...
#ifndef DLT_ETHERNET_MPACKET
#define DLT_ETHERNET_MPACKET 274
#endif // DLT_ETHERNET_MPACKET
...
#define JUNIPER_FLAG_EXT     0x80     // Extension header present
...
typedef struct {
    uint16_t proto;         // protocol
    uint16_t reserved_mbz;  // reserved (MUST be zero)
    uint16_t if_index;      // interface index
    uint16_t type;          // link-layer address type
    uint16_t pkt_type;      // packet type
    uint16_t len;           // link-layer address length
    uint8_t  addr[8];       // link-layer address
...
typedef struct {
    ...
    uint32_t dlt;     // Data Link Type of the captured packet data
} ppi_hdr_t;
...
uint8_t *t2_process_linktype(uint8_t *pktptr, packet_t *packet) __attribute__((__nonnull__(1, 2)));

#endif // T2_LINKTYPE_H_INCLUDED
