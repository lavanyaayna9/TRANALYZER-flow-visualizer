...
#ifndef T2_GTP_H_INCLUDED
#define T2_GTP_H_INCLUDED
...
} __attribute__((packed)) gtpv0Header_t;
...
} __attribute__((packed)) gtpv1Header_t;
...
} __attribute__((packed)) gtpv2Header_t;
...
typedef struct {
    ...
    uint16_t len;        // Message Length
    uint16_t seqnum;   // Sequence number
} __attribute__((packed)) gtpPrmHeader_t;

bool     t2_is_gtp      (uint16_t sport, uint16_t dport)    __attribute__((__const__));
uint8_t *t2_process_gtp (uint8_t *pktptr, packet_t *packet) __attribute__((__nonnull__(1, 2)));

#endif // T2_GTP_H_INCLUDED
