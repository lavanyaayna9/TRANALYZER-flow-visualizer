...
#include <inttypes.h>         // for PRIx16, PRIu8, PRIuFAST8, PRIx8
#include <netinet/in.h>       // for ntohs
#include <stdio.h>            // for snprintf, size_t
#include <string.h>           // for memcpy, strlen

#include "flow.h"             // for STPDSCT, HDOVRN
#include "main.h"             // for T2_SET_STATUS
#include "proto/ethertype.h"  // for ETHERTYPE_AARPn, ETHERTYPE_AOEn, ETHERT...
#include "proto/llc.h"        // for LLC_SAP_NULL, LLC_SAP_LLC, LLC_SAP_IP, ...
#include "t2utils.h"          // for LIKELY, MIN, UNLIKELY
...
inline void t2_pktdesc_add_ethproto(packet_t *pkt, uint16_t proto) {
    switch (proto) {
        ...
        case ETHERTYPE_CGMPn:           t2_pktdesc_add_hdr(pkt, ":cgmp");     break;
        ...
        case ETHERTYPE_QINQD1n:         t2_pktdesc_add_hdr(pkt, ":vlan");     break;
        case ETHERTYPE_QINQD2n:         t2_pktdesc_add_hdr(pkt, ":vlan");     break;
        case ETHERTYPE_REALTEKn:        t2_pktdesc_add_hdr(pkt, ":realtek");  break;
        ...
        default: {
            char unk[13];
            snprintf(unk, 13, ":UNK(0x%04" PRIx16 ")", ntohs(proto));
            t2_pktdesc_add_hdr(pkt, unk);
            T2_SET_STATUS(pkt, STPDSCT);
            break;
        }
...
inline void t2_pktdesc_add_pppproto(packet_t *pkt, uint16_t proto) {
    switch (proto) {
        case PPP_CDPn:        t2_pktdesc_add_hdr(pkt, ":cdp");       break;
