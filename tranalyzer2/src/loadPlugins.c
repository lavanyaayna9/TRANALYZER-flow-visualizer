...
#include <dirent.h>     // for dirent, scandir, alphasort
#include <dlfcn.h>      // for dlerror, dlclose, dlopen, dlsym, RTLD_GLOBAL
#include <errno.h>      // for errno
#include <stdbool.h>    // for false, bool, true
#include <stdlib.h>     // for free, strtoul
#include <string.h>     // for strlen, memcpy, strncmp, strtok, strcat, strcmp
#include <sys/types.h>  // for ssize_t

#include "main.h"       // for pluginList
#include "t2log.h"      // for T2_ERR, T2_PERR, T2_LOG, T2_DBG, T2_WRN
#include "t2utils.h"    // for t2_calloc_fatal, t2_strcpy, UNLIKELY, T2_STRCPY_EXIT, LIKELY, T2_FATAL


// Plugin functions (MUST match order of funcs array)
enum {
    PL_GET_NAME,       // t2PluginName
    PL_GET_VERSION,    // t2PluginVersion
    PL_GET_V_MAJOR,    // t2SupportedT2Major
    PL_GET_V_MINOR,    // t2SupportedT2Minor
    PL_GET_DEPS,       // t2Dependencies
    PL_INIT,           // t2Init
    PL_PRI_HDR,        // t2PrintHeader
    PL_FLOW_GEN,       // t2OnNewFlow
    PL_CLAIM_L2,       // t2OnLayer2
    PL_CLAIM_L4,       // t2OnLayer4
    PL_FLOW_TERM,      // t2OnFlowTerminate
#if PLUGIN_REPORT == 1
    PL_REPORT,         // t2PluginReport
#endif // PLUGIN_REPORT == 1
    PL_MONITORING,     // t2Monitoring
    PL_APP_TERM,       // t2Finalize
    PL_BUF_TO_SINK,    // t2BufferToSink
#if USE_T2BUS == 1
    PL_T2BUS_CB,       // t2BusCallback
#endif // USE_T2BUS == 1
#if REPORT_HIST == 1
    PL_SAVE_STATE,     // t2SaveState
    PL_RESTORE_STATE,  // t2RestoreState
#endif // REPORT_HIST == 1
    PL_NUM_FUNCS,
};
...
#define T2_PREFIX_IS_PLUGIN_NUMBER(s) \
    (isdigit((s)[0]) && isdigit((s)[1]) && \
     isdigit((s)[2]) && (s)[3] == '_')
#define T2_SUFFIX_IS_SO(s) \
    t2_str_has_suffix((s), ".so")


static inline int plugin_filter(const struct dirent *de) {
    ssize_t dLen = strlen(de->d_name);
    if (dLen < 8) return 0; // [0-9]{3}_[A-Za-z0-9_]+.so

    if (!T2_PREFIX_IS_PLUGIN_NUMBER(de->d_name)) return 0;
    if (!T2_SUFFIX_IS_SO(de->d_name)) return 0;

#if USE_PLLIST > 0
    dLen -= 3; // discard '.so' extension

    if (!pllist_file) return 1;

    rewind(pllist_file);

    // The plugin loading list supports different formats:
    //  - [0-9]{3}_[A-Za-z0-9_]+.so -> 100_basicFlow.so
    //  - [A-Za-z0-9_]+             -> basicFlow

    size_t len = 0;
    ssize_t read;
    char *line = NULL;
    while ((read = getline(&line, &len, pllist_file)) != -1) {
        if (line[0] == '#' || read < 1) continue; // skip comments

        if (line[read - 1] == '\n') line[--read] = '\0';

        // discard '.so' extension
        if (T2_SUFFIX_IS_SO(line)) {
            line[read - 3] = '\0';
            read -= 3;
        }

        // if available, include plugin number in comparison
        const uint8_t offset = ((read > 4 && T2_PREFIX_IS_PLUGIN_NUMBER(line)) ? 0 : 4);

        if (read == (dLen - offset) && memcmp(de->d_name + offset, line, dLen - offset) == 0) {
#if USE_PLLIST == 1
            free(line);
            return 1;
#else // USE_PLLIST == 2
...
static inline bool validate_plugin_name(const char *filename, const char *name) {
...
static inline bool validate_version_num(const char *name, unsigned int v_major, unsigned int v_minor) {
    bool valid = true;
...
static inline bool validate_version_str(const char *name, const char *version) {
...
static inline bool validate_plugin_version(const char *name, const char *version, unsigned int v_major, unsigned int v_minor) {
    bool valid = validate_version_str(name, version);
    valid &= validate_version_num(name, v_major, v_minor);
    return valid;
}
...
static bool check_deps(const char *dependencies, t2_plugin_t plugins[], uint8_t num_plugins) {
    ...
    t2_strcpy(deps, dependencies, sizeof(deps), T2_STRCPY_EXIT);
...
t2_plugin_array_t* t2_load_plugins(const char *folder) {
    const size_t dirlen = strlen(folder);
    if (dirlen > PL_PATH_MAXLEN) {
        T2_FATAL("Plugin path too long (limited to %u)", PL_PATH_MAXLEN);
    }
    ...
    if (n < 0) {
        T2_FATAL("Failed to scan plugin folder '%s': %s", folder, strerror(errno));
    }
    ...
    t2_plugin_array_t *plugins = t2_calloc_fatal(1, sizeof(*plugins));
    plugins->plugin = t2_calloc_fatal(n, sizeof(t2_plugin_t));
    ...
    t2_strcpy(filename, folder, sizeof(filename), T2_STRCPY_EXIT);
    ...
    for (int i = 0; i < n; i++) {
        ...
        // load plugin
        t2_strcpy(filename + dirlen, plugin_name, sizeof(filename)-dirlen, T2_STRCPY_EXIT);
        ...
        if (!validate_plugin_name(plugin_name, name)) {
            name = plugin_name;
            abort = true;
        }
        ...
        abort |= !validate_plugin_version(name, version, v_major, v_minor);
        ...
        memcpy(plugins->plugin[num_plugins].name, name, strlen(name) + 1);
        memcpy(plugins->plugin[num_plugins].version, version, strlen(version) + 1);
    ...
    if (num_plugins != plugins->num_plugins) {
        ...
        T2_REALLOC(plugins->plugin, num_plugins * sizeof(t2_plugin_t));
    }
