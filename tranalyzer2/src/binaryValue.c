...
#include "t2utils.h"  // for t2_[cm]alloc, UNLIKELY, MIN, LIKELY, MAX, T2_FATAL


// Double the buffer size
#define BV_HDR_RESIZE(header, capacity, required) \
    if (UNLIKELY((required) > (capacity))) { \
        do { \
            (capacity) <<= 1; \
        } while (UNLIKELY((required) > (capacity))); \
        if (UNLIKELY(capacity >= HDRBUF_MAX_SZ)) { \
            T2_FATAL("Binary header overflow. Try increasing HDRBUF_MAX_SZ in binaryValue.h"); \
        } \
        T2_REALLOC((header), (capacity) * sizeof(uint32_t)); \
    }
 ...
 */
inline binary_value_t *bv_append_bv(binary_value_t *dest, binary_value_t *new_bv) {
    if (!dest) return new_bv;
...
binary_value_t *bv_add_sv_to_bv(binary_value_t *dest, uint32_t pos, uint32_t is_repeating, uint32_t num_values, ...) {
    if (UNLIKELY(pos >= dest->num_values)) return NULL;

    dest->subval[pos].type = 0;
    dest->subval[pos].is_repeating = is_repeating;
    dest->subval[pos].num_values = num_values;
    dest->subval[pos].subval = t2_calloc_fatal(num_values, sizeof(binary_subvalue_t));
 ...
 */
binary_value_t *bv_new_bv(const char *name, const char *desc, uint32_t is_repeating, uint32_t num_values, ...) {
    binary_value_t *bv = t2_calloc_fatal(1, sizeof(binary_value_t));
    bv->subval = t2_calloc_fatal(num_values, sizeof(binary_subvalue_t));
...
binary_header_t *build_header(binary_value_t *bv) {
    ...
    binary_header_t *header = t2_malloc_fatal(sizeof(*header));
    header->header = t2_calloc_fatal(capacity, sizeof(uint32_t));
    ...
    header->length = BUF_DATA_SHFT + 3; // Magic (2), Version (1)
    ...
    while (bvp != NULL) {
        ...
        const uint32_t required = header->length + size + BV_END_MARKER_SIZE;
        BV_HDR_RESIZE(header->header, capacity, required);
    ...
    header->header[header->length++] = BV_END_MARKER;

    if (header->length != capacity) {
        // get rid of unused memory
        T2_REALLOC(header->header, header->length * sizeof(uint32_t));
    }
...
static binary_header_t *rek_build_header(binary_subvalue_t *sv, binary_header_t *header, uint32_t *capacity) {
    const uint32_t required = header->length + (sv->num_values + 2) + BV_END_MARKER_SIZE;
    BV_HDR_RESIZE(header->header, *capacity, required);
