...
#ifndef T2_BINARYVALUE_H_INCLUDED
#define T2_BINARYVALUE_H_INCLUDED

#include <stdint.h>       // for uint32_t

#include "outputBuffer.h" // for BUF_DATA_SHFT
...
#define HDRBUF_MIN_SZ  (BUF_DATA_SHFT + 4)                    // Magic (2), Version (1), End Marker (1)
#define HDRBUF_INIT_SZ 8192                                   // Initial size of the header buffer
#define HDRBUF_MAX_SZ  MIN((HDRBUF_INIT_SZ * 64), UINT32_MAX) // Max size of the header buffer
...
// The end marker should be at the end of every tranalyzer binary header
#define BV_END_MARKER      UINT32_MAX
#define BV_END_MARKER_SIZE 1
...
#define BV_APPEND_FULL(bv, name, desc, repeat, num_val, args...) \
    (bv) = bv_append_bv((bv), bv_new_bv(name, desc, repeat, num_val, ##args))
...
binary_value_t* bv_append_bv(binary_value_t *dest, binary_value_t *new_bv);
...
#endif // T2_BINARYVALUE_H_INCLUDED
