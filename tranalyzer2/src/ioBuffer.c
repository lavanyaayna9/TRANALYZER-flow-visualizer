...
#include <pcap/pcap.h>      // for pcap_dispatch, pcap_geterr, pcap_close, pcap_pkthdr, ...
#include <pthread.h>        // for pthread_t, pthread_mutex_t, pthread_create, ...
#include <stdio.h>          // for fflush
#include <string.h>         // for strlen, memcpy, strerror
#include <sys/stat.h>       // for stat
#include <unistd.h>         // for sleep, usleep
#include <wordexp.h>        // for wordexp, wordexp_t, wordfree

#include "flow.h"           // for PCAPSNPD
#include "main.h"           // for dooF, capType, capName, captureDescriptor, globalInt
#include "packetCapture.h"  // for perPacketCallback
#include "t2log.h"          // for T2_LOG, T2_WRN, T2_ERR
#include "t2utils.h"        // for UNLIKELY, LIKELY, UNUSED, T2_FATAL
#include "tranalyzer.h"     // for VERBOSE, RROP, MFPTMOUT
...
static void ioBufferPush(u_char *inqueue UNUSED, const struct pcap_pkthdr *pcapHeader, const u_char *packet);
...
inline void ioBufferInitialize() {
    ...
    if (UNLIKELY(pthread_create(&thread, NULL, ioBufferThreadFunc, NULL))) {
        T2_FATAL("Failed to create thread for ioBuffer");
    }
}


static void *ioBufferThreadFunc(void *args UNUSED) {

    char errbuf[PCAP_ERRBUF_SIZE];

    while (LIKELY((globalInt & GI_RUN) > GI_EXIT)) {

        const int pcap_ret = pcap_dispatch(captureDescriptor, PACKETS_PER_BURST, ioBufferPush, NULL);
...
nxtnumB:
        ...
        // -R option
            ...
            T2_LOG("Link layer type: %s [%s/%d]", dl_descr, dl_name, linkType);
            T2_LOG("Snapshot length: %d", snaplen);
            fflush(dooF);
#endif // VERBOSE > 1
