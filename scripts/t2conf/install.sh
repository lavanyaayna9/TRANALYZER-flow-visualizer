...
usage() {
    ...
    echo
    echo "Target:"
    echo "    deps       install dependencies (dialog)"
    echo "    man        install the man page in /usr/local/man/man1"
    echo "    t2conf     install an alias for t2conf"
    echo "    all        install deps, t2conf and man"
    echo
...
get_rc_files() {
    ...
    if [[ "${#rcfiles[@]}" == 0 ]]; then
        if [[ -z "${DOCKER_BUILD}" ]]; then
            ...
            printinf "Copy the following alias to your rc file: alias t2conf=\"${SHOME}/t2conf\""
            exit 1
        else
            rc="${HOME}/.bashrc"
            if [[ ! -f "${rc}" ]]; then
...
install_t2conf() {
    ...
    for rc in "${destrc[@]}"; do
        if grep -qF 'alias t2conf=' "${rc}"; then
        ...
        if ! echo "alias t2conf=\"${SHOME}/t2conf\"" >> "${rc}"; then
...
uninstall_t2conf() {
    ...
    for rc in "${destrc[@]}"; do
        if grep -qF 'alias t2conf=' "${rc}"; then
            ...
            if ! "${SED}" -i '/alias t2conf=".*"$/d' "${rc}" || grep -qF 'alias t2conf=' "${rc}"; then
...
while [[ "${#}" -gt 0 ]]; do
    case "${1}" in
        ...
        all) TO_INSTALL+=(deps man t2conf);;
