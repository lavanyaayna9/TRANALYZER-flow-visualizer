...
Alternatively, if you prefer to run Tranalyzer yourself or already have access to a flow file, replace step 3 of \refs{t2fm-pcap-pdf} with the following steps:
\begin{enumerate}
...
If the \tranrefpl{clickhouseSink}, \tranrefpl{mongoSink} or \tranrefpl{psqlSink} plugins were loaded, t2fm can use the created databases to generate the report (faster).
\begin{enumerate}
    ...
    \item Build the \tranrefpl{clickhouseSink}, \tranrefpl{mongoSink} or \tranrefpl{psqlSink} plugin:
        \begin{itemize}
